...
class GoogleVertexDocumentStore(BaseDocumentStore):
    ...
    ** Note: This implementation supports the Google Vertex nearest neighbour search based document store. **
    ...
    def _create_index_client(self):
        index_client = aiplatform_v1beta1.IndexServiceClient(
            client_options=client_options.ClientOptions(api_endpoint=self.endpoint)
        )
        return index_client

    #Google
    def _get_index_config(self, config_name): #Please read the documentation to understand the various configuration parameters that can be used to tune the index
        ...
        config = struct_pb2.Struct(
            fields={
                "dimensions": struct_pb2.Value(number_value=self.vector_dim),
        ...
        return gca_index.Index(ann_index)

    #Google
    def create_ann_index(self):
        index_client = self._create_index_client()
        index_resource = index_client.create_index(parent=self.parent, index=self._get_index_config(""))
        while True:
            if index_resource.done():
                break
    ...
    #Google
    def create_brute_force_index(self):
        index_client = self._create_index_client()
        index_resource = index_client.create_index(parent=self.parent, index=self._get_index_config(""))
        while True:
            if index_resource.done():
